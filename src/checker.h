      ...
      for (auto &def : node.defList) {
        ...
        if (!def->arrays.empty()) {
          ...
          for (auto &exp : def->arrays) {
            ...
            // 例如 int a[0][1.2]; 这种情况
            if (!Expr_int) {
              err.error(ErrorType::ArrayIndexNotInt, *def->id,def->m_line);
        ...
        if (!result.second) {
          err.error(ErrorType::VarDuplicated, *def->id,def->m_line);
          return false;
        }
      }
    }
  ...
  bool InsertFunc(const FuncDefAST &node) {
    ...
    if (!node.funcFParamList.empty()) {
      for (auto &param : node.funcFParamList) {
        ...
        //函数形参重复定义
        if (!result.second) {
          err.error(ErrorType::VarDuplicated, *param->id,param->m_line);
    ...
    if (checkFuncEntry != table.front().end()) {
      ...
      if (checkFuncEntry->second.is_func) {
        err.error(ErrorType::FuncDuplicated, *node.id, node.m_line);
