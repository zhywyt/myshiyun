...
void Checker::visit(DeclAST &ast) {
  ...
  if (!InsertVar(ast)) {
    // TODO 变量重复定义
    throw(int(ErrorType::VarDuplicated));//finished
  }
}
...
void Checker::visit(ReturnStmtAST &ast) {
  ...
  if (entry->type != this->current_type.type) {
    err.error(ErrorType::FuncReturnTypeNotMatch,"",ast.m_line);
    throw(int(ErrorType::FuncReturnTypeNotMatch));//finished
...
void Checker::visit(FuncDefAST &ast) {
  ...
  // TODO 函数重复定义
  if (!InsertFunc(ast)) {
    err.error(ErrorType::FuncDuplicated, *ast.id,ast.m_line);
    throw(int(ErrorType::FuncDuplicated));//finished
  ...
  if (ast.sType == STYPE::BRE) { // 当前节点为Break语句类型
    // TODO 检查该break语句是否在循环体内,若不是应报错
      if(this->in_loop==false){
        err.error(ErrorType::BreakNotInLoop, "",ast.m_line);
        throw(int(ErrorType::BreakNotInLoop));//finished
    }
    ...
    // TODO 检查该continuek语句是否在循环体内,若不是应报错
      if(this->in_loop==false){
        err.error(ErrorType::ContinueNotInLoop, "",ast.m_line);
        throw(int(ErrorType::ContinueNotInLoop));//finished
    }
  }
...
void Checker::visit(LValAST &ast) {
  ...
  if (entry == nullptr) {
    // TODO ERROR
    // 使用未定义变量
    err.error(ErrorType::VarUnknown, *ast.id,ast.m_line);
    throw(int(ErrorType::VarUnknown));//finished
  }
  ...
  if (!entry->is_array && !ast.arrays.empty()) {
    err.error(ErrorType::VisitVariableError, *ast.id, ast.m_line);
    throw(int(ErrorType::VisitVariableError));//finished
...
void Checker::visit(CallAST &ast) {
  ...
  if (entry == nullptr) {
    // TODO ERROR
    // 函数未定义
    err.error(ErrorType::FuncUnknown, *ast.id,ast.m_line);
    throw(int(ErrorType::FuncUnknown));//finished
    ...
    if (entry->func_params.size() != ast.funcCParamList.size()) {
      // TODO ERROR
      // 函数参数/类型不匹配
      err.error(ErrorType::FuncParamsNotMatch, *ast.id,ast.m_line);
      throw(int(ErrorType::FuncParamsNotMatch));//finished
